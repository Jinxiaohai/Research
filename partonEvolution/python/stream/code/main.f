...
clin-3/2015 Introduce parton density fluctutations for String Melting:
...
clin-3/2015:
      ...
      READ (24, *) rtcluster
      READ (24, *) icluster
      if(ncluster.gt.100) then
         write(99,*) 'ncluster cannot be bigger than 100'
         stop
      endif
      if(icluster.ne.0.and.icluster.ne.1) then
         write(99,*) 'icluster needs to be either 0 or 1'
         stop
      endif
c
      CLOSE (24)
...
c
      if(isoft.eq.1) then
         amptvn = '1.26t5b (Default)'
      elseif(isoft.eq.4) then
         amptvn = '2.26t5b (StringMelting)'
      else
         amptvn = 'Test-Only'
      endif
      WRITE(6,50) amptvn
...
c     AMPT momentum and space info at freezeout:
      ...
      OPEN (38, file = 'ana/after_parton.dat', status = 'unknown')
