...
int ampt()
{
  ...
  pad->Draw();
  pad->cd();
  ifstream input("../../code/ana/ampt.dat");
  ...
  unsigned int nevent = 0, nrun = 0, multi = 0,
    npartP = 0, npartT = 0, nelp = 0, ninp = 0, nelt = 0, nint = 0;
  ...
  input >> nevent >> nrun >> multi >> impactpar >> npartP >> npartT >>
          ...
          if (cell[i][j][3] > 8)
            {
              ...
              double endx = starx + length*cos(cell[i][j][2])/15;
              double endy = stary + length*sin(cell[i][j][2])/15;
