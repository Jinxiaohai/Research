...
void parseHepMC_Cumulants(char *filename = "./ana/ampt.dat",
                          char* outFileName = "ampt_correlation_out.root")
{
  ...
  for (int fnum = 0; fnum < 1; fnum++)
    {
      ...
      ifstream myFile;
      myFile.open(Form("./ampt.dat", fnum));
      ...
      while (myFile)
        {
          ...
          myFile >> evtnumber >> testnum >> nlist >> impactpar >> npartproj
                 >> nparttarg >> npartprojelas >> npartprojinelas >> nparttargelas
                 >> nparttarginelas >> junk;
          ...
          for (int i = 0; i < nlist; i++)
            {
              ...
              float energy =  sqrt (pow(pvec[0], 2) + pow(pvec[1], 2)
                                    + pow(pvec[2], 2) + pow(mass, 2));
