		  ...
		  for(int k=0;k<=8;k++)
		    {
			  ...
			  if((k+h)>8)	continue;
			  if((i+j+k+h)>8) continue;
			  D[i][j][k][h] = DD(4,meanV,F,i,j,k,h)/u3-(u4-3*pow(u2,2.))*DD(3,meanV,F,i,j,k,h)/
			    pow(u3,2.)-6*u2*DD(2,meanV,F,i,j,k,h)/u3;
			}
		    }
		}
	    }
				  ...
				  for(int v=0;v<=8;v++)
				    {
				      ...
				      double eff=1/(pow(averEff_low[cent],i+r)*pow(averEff_low[cent],j+s)*
						    pow(averEff_low[cent],k+u)*pow(averEff_low[cent],h+v));
	  ...
	  for(int j=0;j<=8;j++)
	    {
	      if((i+j)<=8)
	  	{
		  F[i][j]=(((TProfile *)f->Get(Form("F%d%d_Cent%d",i,0,cent)))->GetBinContent(ref))*
		    (((TProfile *)f->Get(Form("F%d%d_Cent%d",0,j,cent)))->GetBinContent(ref));
		}
	    }
	}
      TotalEvents+=Events;
...
double Moments_IndProd_Errors(const char *name, const char *name2, TFile *f, int cent)
{
  ...
  for(int ref=1;ref<=1000;ref++)
      ...
      if(strcmp(name2,"C4")==0)
	{
	  ...
	  double var_c4=pow(var,4)*(99 + 42*(NCM[4]-3) - (NCM[4]-3)*(NCM[4]-3) -
				    12*NCM[6] + NCM[8] - 8*NCM[5]*NCM[3] + 64*NCM[3]*NCM[3])/Events;
      ...
      if(strcmp(name2,"KV")==0)
	{
	  ...
	  double var_KV=pow(var,2)*(NCM[8]-2*(NCM[4]+3)*NCM[6]-8*NCM[3]*NCM[5]+8*pow(NCM[3],2)*
				    (NCM[4]+5)+(pow(NCM[4]-3,3)+15*pow(NCM[4]-3,2)+72*(NCM[4]-3)+99))/Events;
      ...
      if(strcmp(name2,"KDS")==0)
	{
	  ...
	  double var_KDS=var*((-6*pow(NCM[4]-3,3) + 2*(NCM[4]-3)*(9*NCM[5] - NCM[7])*NCM[3]
			       + (NCM[4]-3)*(NCM[4]-3)*(-9 + NCM[6] + 8*NCM[3]*NCM[3])+NCM[3]*NCM[3]*
			       (99 - 12*NCM[6] + NCM[8] - 8*NCM[5]*NCM[3] + 64*NCM[3]*NCM[3])))/
	    pow(NCM[3],2)/pow(NCM[3],4)/Events;
      ...
      for(int i=0; i<=8; i++)
	{
      ...
      if(strcmp("netp",name)==0)
 	{
	  ...
	  Cum[3]+=(eps_p*F[1][0]*(2*eps_p-1)-eps_pbar*F[0][1]*(2*eps_pbar-1))*Events;
 	  Cum[4]+=(eps_p*F[1][0]*(6*eps_p*eps_p-6*eps_p+1)+eps_pbar*F[0][1]*
		   (6*eps_pbar*eps_pbar-6*eps_pbar+1))*Events;
 	}
      if(strcmp("totp",name)==0)
 	{
	  Cum[1]+=TotalV*Events;
		  ...
		  for(int y1=0;y1<=r;y1++)
		      ...
		      for(int y2=0;y2<=s;y2++)
			{
			  for(int y3=0;y3<=u;y3++)
				...
				{
				  count+=StirlingS1(i,x1)*StirlingS1(j,x2)*StirlingS1(k,x3)*StirlingS1(h,x4)*
				    StirlingS1(r,y1)*StirlingS1(s,y2)*StirlingS1(u,y3)*StirlingS1(v,y4)*
				    CovMoment4(x1,x2,x3,x4,y1,y2,y3,y4,FF);
				}
			    }
			}
		    }
	  ...
	  for(int s=0;s<=i1;s++)
	    {
	      for(int t=0;t<=i2;t++)
		{
		  count+=StirlingS1(r1,i1)*StirlingS1(r2,i2)*C(i1,s)*C(i2,t)*StirlingS2(i1-s,u)*
		    StirlingS2(s,v)*StirlingS2(i2-t,j)*StirlingS2(t,k);
		}
	    }
	}
    }
