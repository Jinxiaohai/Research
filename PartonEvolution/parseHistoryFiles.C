...
void draw()
{
  ...
  TCanvas *cScatteringAngle = new TCanvas("cScatteringAngle","cScatteringAngle",
                                          500,500);
...
void parseHistoryFiles(char *initialInfoFile = "parton-initial-afterPropagation.dat",
                       char *evolInfoFile = "parton-collisionsHistory.dat",
                       char *outputFile = "evolution_out.root")
{
  ...
  for(int i=0; i<NSCATT; i++)
    {
      hDeltaRapidity[i] = new TH1F(Form("hDeltaRapidity_%i",i),
                                   Form("hDeltaRapidity_%i;#Delta y",i),500,-10,10);
      hRapidity[i] = new TH1F(Form("hRapidity_%i",i),
                              Form("hRapidity_%i;y",i),500,-5.01,4.99);
      hdN_dpT[i] = new TH1F(Form("dN_dpT_%i",i),
                            Form("dN_dpT_%i;p_{T};dN/dp_{T}",i),100,0,4);
      hv2_pT[i] = new TProfile(Form("hv2_pT_%i",i),
                               Form("hv2_pT_%i;p_{T};v_{2}",i),50,0,5,-2,2);
      hv2_pT_mideta[i] = new TProfile(Form("hv2_pT_mideta_%i",i),
                                      Form("hv2_pT_mideta_%i;p_{T};v_{2}",i),50,0,5,-2,2);
      hAngle[i] = new TH1F(Form("hAngle_%i",i),
                           Form("hAngle_%i;#theta",i),500,0,2*TMath::Pi());
      hEndStatev2[i] = new TH2F(Form("hEndStatev2_%i",i),
                                Form("hEndStatev2_%i",i),1200,-1.2,1.2,50,0,5);
      hv2_pT_finalstate = new TProfile("hv2_pT_finalstate",
                                       "hv2_pT_finalstate",50,0,5,-2,2);
    }
  ...
  while (myFileInitialInfo)
    {
      ...
      myFileInitialInfo >> evtnumber >> iterindex >> nlist >> nbaryons
                        >> nmesons >> nparticles >> nparticleszpc;
      ...
      for (int i=0;i<nlist;i++)
        {
          ...
          myFileInitialInfo >> partid >> pvec[0] >> pvec[1] >> pvec[2]
                            >> mass >> spacetime[0] >> spacetime[1]
                            >> spacetime[2] >> spacetime[3];
      ...
      string regexp = Form(" event,miss,iscat,jscat= %i [0-9]+ [0-9]+ [0-9]+",
                           evtnumber);
      ...
      while(myFileEvolInfo)
	{
	  ...
	  headerstream >> description >> junk1 >> junk2
                       >> partonindex1 >> partonindex2;

	  myFileEvolInfo >> partid_init_1 >> pvec_init_1[0] >> pvec_init_1[1]
                         >> pvec_init_1[2] >> mass_init_1 >> spacetime_init_1[0]
                         >> spacetime_init_1[1] >> spacetime_init_1[2]
                         >> spacetime_init_1[3];	
	  
	  myFileEvolInfo >> partid_init_2 >> pvec_init_2[0] >> pvec_init_2[1]
                         >> pvec_init_2[2] >> mass_init_2 >> spacetime_init_2[0]
                         >> spacetime_init_2[1] >> spacetime_init_2[2]
                         >> spacetime_init_2[3];	 
	  
	  myFileEvolInfo >> partid_final_1 >> pvec_final_1[0] >> pvec_final_1[1]
                         >> pvec_final_1[2] >> mass_final_1 >> spacetime_final_1[0]
                         >> spacetime_final_1[1] >> spacetime_final_1[2]
                         >> spacetime_final_1[3];
	  
	  myFileEvolInfo >> partid_final_2 >> pvec_final_2[0] >> pvec_final_2[1]
                         >> pvec_final_2[2] >> mass_final_2 >> spacetime_final_2[0]
                         >> spacetime_final_2[1] >> spacetime_final_2[2]
                         >> spacetime_final_2[3];	 
