...
double_complex chyper(double_complex a,double_complex b,double_complex cz,int kk);
...
{
  int kk;
  kk = static_cast<int>(floor((static_cast<double>(nkmax))*mom/maxmom));
  /* kk=(int)floor((double)nkmax*mom/maxmom); */
  /// 貌似更改数据的值(分为分子和分母两种形式)。
  if(kk<nkmax)
    {
      if(inumden==0)
        {
...
void onedbin_class::printcorr(int onlyzerob)
{
  ...
  if(normcount>0)
    {
     normalization=normalization/(double)normcount; 
    }
      ...
      if(numcounter[kk]!=0 && (no_denominator==1 || dencounter[kk]!=0))
        {
          ...
          if(no_denominator!=1)
            {
              error=error+corr*corr/(double)numcounter[kk]; 
            }
          if(no_denominator!=1)
            {
              ...
              if(no_denominator!=1)
                {
                  error=error+corr*corr/(double)dencounter[kk]; 
                }
            }
...
void threedbin_class::bincorr(int inumden,double kx,double ky,double kz,double corr)
{
  int kk,ix,iy,iz;
