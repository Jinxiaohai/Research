#ifndef __INCLUDE_SOURCECALC_BLAST
#define __INCLUDE_SOURCECALC_BLAST
//#define __CALC_SCBLAST_GAUSSPARS__
...
void CSourceCalc_Blast::CalcS(CCHArray *A){
  ...
  double ma,mb,Pt,T,tau,volume,delr;
  int nsample;
  double rcm[4],**ra,**rb;
  ...
  delete ra;
  if(!SameMass) delete rb;

}

void CSourceCalc_Blast::Get_r(double *p,double **r){
  double eta,arg,etaG,u[4],x,y,z,t,R,weight;
  ...
  for(imc=0;imc<nsample;imc++){
    ...
    r[imc][0]=gamma*t-gammav*x;
    r[imc][1]=gamma*x-gammav*t;
    r[imc][2]=y;
    r[imc][3]=z;
  }
}
...
double CSourceCalc_Blast::GetTau(double tau0,double deltau){
  double Pg,Px,Ptot,tau,taux;
  const double PI=4.0*atan(1.0);
  Px=deltau*deltau;
  Pg=tau0*deltau*sqrt(2.0*PI);
  Ptot=Pg+Px;
  Pg=Pg/Ptot;
  Px=Px/Ptot;
 STARTOVER:
  if(randy->ran()<Pg){
    ...
    if(tau<0.0) goto STARTOVER;
    if(tau<tau0 && randy->ran()>tau/tau0) goto STARTOVER;
  }
  else{
    tau=tau0+deltau*sqrt(-2.0*log(randy->ran()));
  }
  return tau;
}
