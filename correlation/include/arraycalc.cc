...
void ArrayCalc::CalcAExpArrayFromMArray(CCHArray *M,int irm,
					CCHArray *A,int ira){
...
void ArrayCalc::CalcMArrayFromAExpArray(CCHArray *A,int ira,
				   CCHArray *M,int irm){
  int LMAX=A->GetLMAX();
  if(M->GetLMAX()!=LMAX){
    printf("LMAX from M does not match LMAX from A\n");
...
void ArrayCalc::AddArrays(CCHArray *A,int ira,CCHArray *B,
...
void ArrayCalc::CalcYlmExpArrayFromAExpArray(CCHArray *A,int ira,
...
void ArrayCalc::CalcAExpArrayFromYlmExpArray(CYlmArray *YlmArray,int irlm,
					     CCHArray *A,int ira){
  CCHCalc chcalc;
  const double PI=4.0*atan(1.0);
  int LMAX,L,lx,ly,lz,M,k;
...
void ArrayCalc::Detrace(CCHArray *M,int irm,CCHArray *A,int ira){
  ...
  A->ZeroArray(ira);
  int L,LMAX,lx,ly,lz,m,mx,my,mz,mxfact,myfact,mzfact,lxprime,lyprime,lzprime;
  int LMAXM=M->GetLMAX();
  int LMAXA=A->GetLMAX();
  LMAX=LMAXM;
...
void ArrayCalc::MultiplyArrays(CCHArray *A,int ira,CCHArray *B,
			       int irb,CCHArray *C,int irc){
  int LMAXA=A->GetLMAX();
  int LMAXB=B->GetLMAX();
  int LMAXC=C->GetLMAX();
...
void ArrayCalc::MultiplyArrays_Partial(int LMAXA,CCHArray *A,int ira,
...
void ArrayCalc::DivideArrays(CCHArray *A,int ira,
				 CCHArray *B,int irb,
				 CCHArray *C,int irc){
...
void ArrayCalc::CalcAExpArrayFromXExpArray(CCHArray *X,int irx,
					    CCHArray *A,int ira){

  double r,snorm,biggy,btest;
...
void ArrayCalc::CalcXExpArrayFromAExpArray(CCHArray *A,int ira,
					   CCHArray *X,int irx){
...
void ArrayCalc::CalcAExpArrayFrom3DArray(C3DArray *threed,CCHArray *A){
  int lx,ly,lz,L,ir,ix,iy,iz;
  double x,y,z,r,cthetamin,phimin,phimax,delx,dely,delz,fx,fy,fz,ex,ey,ez;
  double ctheta,stheta,phi,interpolate;
  const double PI=4.0*atan(1.0);
  int ictheta,iphi,nctheta,nphi;
  bool XSYM,YSYM,ZSYM;
  ...
  for(ir=0;ir<A->GetNRADIAL();ir++){
	...
	if(fabs(x)<delx*threed->GetNXMAX() && fabs(y)<dely*threed->GetNYMAX()
	  ...
	  interpolate=threed->GetElement_Interpolate(x,y,z);
	  ex=x/r;
	  ey=y/r;
	  ez=z/r;
...
void ArrayCalc::Calc3DArrayFromAExpArray(CCHArray *A,C3DArray *threed){
  int ix,iy,iz,isx,isy,isz,nsx,nsy,nsz,ira,irb;
  double r,ra,rb,valuea,valueb,interpolate,f,x,y,z,ex,ey,ez;
...
void ArrayCalc::MultiplyArrays(C3DArray *threeda,C3DArray *threedb,
				C3DArray *threedc){
...
void ArrayCalc::DivideArrays(C3DArray *threeda,C3DArray *threedb,
				  C3DArray *threedc){
  int ix,iy,iz,isx,isy,isz,nsx,nsy,nsz;
  if(!CompareArrayParameters(threeda,threedb)) exit(1);
  if(!CompareArrayParameters(threeda,threedc)) exit(1);
...
void ArrayCalc::AddArrays(C3DArray *threeda,C3DArray *threedb,
			       C3DArray *threedc){
  int ix,iy,iz,isx,isy,isz,nsx,nsy,nsz;
  if(!CompareArrayParameters(threeda,threedb)) exit(1);
  if(!CompareArrayParameters(threeda,threedc)) exit(1);
