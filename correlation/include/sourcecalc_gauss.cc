#ifndef __INCLUDE_SOURCECALC_GAUSS
#define __INCLUDE_SOURCECALC_GAUSS
...
void CSourceCalc_Gaussian::InitSPars(){
  ...
  parameter::set(spars,"Xoff",0);
  parameter::set(spars,"Yoff",0);
  parameter::set(spars,"Zoff",0);
  parameter::set(spars,"Euler_Phi",0);
  parameter::set(spars,"Euler_Theta",0);
  parameter::set(spars,"Euler_Psi",0);  
}
...
void CSourceCalc_Gaussian::CalcS(CCHArray *A){
  ...
  int GNMAX,NRADIAL;
  double **alpha;
  double r,snorm,biggy,btest,DELR,norm;
  CCHArray *Bn,*b,*bb,*C,*oldC;
  bool XSYM,YSYM,ZSYM;
  XSYM=A->GetXSYM();
  YSYM=A->GetYSYM();
  ZSYM=A->GetZSYM();
