...
/// 定义绝对的最大动量值????如果MIXED_PAIRS_FOR_DENOM进行了定义,
/// 下面定义的绝对值将会被忽略.
#define ABSOLUTE_MAXMOM 100


/**
 * 包含两种的binnings,其中一维的和三维的关联函数形式.
 * 一维的得到C(x), 三维的得到C(x, y, z).
 */
...
/**
 * 对于其中包含的一个子函数decompose(p1, p2, vs, long_comoving_boost,
 *                                   x_along_p, outwards_boost, &kmag,
 *                                   &kx, &ky, &kz);
 * 该程序包含三维的相对动量,当该函数被调用的时候,其中long_comoving_boost,
 * x_along_p, outwards_boost这三个参数决定了reference frame.
 * (1) long_comoving_boost = 1.则沿着束流的方向boost到pz1+pz2=0
 *     的坐标系中.如果long_comoving_boost != 1,则longitudinal frame
 *     将会设定为vs.
 * (2) x_along_p = 1,则进行旋转,在反应平面定义粒子对的动量,
 *     使x坐标轴平行于总的动量的方向.
 * (3) outwards_boost = 1,沿着p1 + p2的质心系的方向进行
 *     boost.
 */
...
/**
 * A: 依据Qinv的关联函数.
 * B: 依据qinv的关联函数,但是是在十个不同的pt bin中.
 * C: 依据|Q|的关联函数,其中|Q|通过longitudinally comoving frame决定.
 * D: 依据Qx, Qy, Qz得到三维的关联函数.
 */
