...
void calc_flow(TString inputFileName="test.root")
{
  ...
  if (!inFile->IsOpen()) cout<<"File Open Error"<<endl;
  ...
  for (Int_t i_loop = 0; i_loop < 2; ++i_loop)
    {
      ...
      pRefCorrelations[i_loop] = (TProfile*)inFile->Get(TString("refCorrelations").Append(subPostfix).Data());
      ...
      for (Int_t i = 0; i < 5; ++i)
        {
          ...
          subEvtEffEntries[i] = TMath::Power(pSubEventRefCorr->GetBinEntries(iBin), 2) / hSubEventSumW2->GetBinContent(iBin);
          subEvtMeanErr2[i]   = TMath::Power(pSubEventRefCorr->GetBinError(iBin), 2) / (subEvtEffEntries[i]- 1);
        }
      ...
      for ( Int_t i = 0; i < 3; ++i )
        {
          ...
          effEntries_ref[i] = TMath::Power(pRefCorrelations[i_loop]->GetBinEntries(iBin), 2) / hRefBinSumW2[i_loop]->GetBinContent(iBin); // \sum{w_i}^2 / \sum{w_i^2}
          meanErr2_ref[i]   = TMath::Power(pRefCorrelations[i_loop]->GetBinError(iBin), 2) / (effEntries_ref[i] - 1);
        }
              ...
              for ( Int_t iEta = 0; iEta < 20; ++iEta )
                {
                  ...
                  effEntries_diff[0] = TMath::Power(pC14[i_loop]->GetBinEntries(globalBinNum), 2) / hDiffBinSumW12[i_loop]->GetBinContent(globalBinNum); // \sum{w_i}^2 / \sum{w_i^2}
                  meanErr2_diff[0]   = TMath::Power(pC14[i_loop]->GetBinError(globalBinNum), 2) / (effEntries_diff[0] - 1);

                  mean_diff[1]       = pC15[i_loop]->GetBinContent(globalBinNum);
                  effEntries_diff[1] = TMath::Power(pC15[i_loop]->GetBinEntries(globalBinNum), 2) / hDiffBinSumW12[i_loop]->GetBinContent(globalBinNum); // \sum{w_i}^2 / \sum{w_i^2}
                  meanErr2_diff[1]   = TMath::Power(pC15[i_loop]->GetBinError(globalBinNum), 2) / (effEntries_diff[1] - 1);
	  
                  mean_diff[2]       = pT28[i_loop]->GetBinContent(globalBinNum);
                  effEntries_diff[2] = TMath::Power(pT28[i_loop]->GetBinEntries(globalBinNum), 2) / hDiffBinSumW22[i_loop]->GetBinContent(globalBinNum); // \sum{w_i}^2 / \sum{w_i^2}
                  meanErr2_diff[2]   = TMath::Power(pT28[i_loop]->GetBinError(globalBinNum), 2) / (effEntries_diff[2] - 1);
