...
int main(int argc, char** argv)
{
  ...
  for (Int_t i = 0; i < 2; ++i)
    {
      h3_mP[i] = new TH3F((TString("yield_sub")+=i).Data(), "",
                          5, 0, 5, 40, 0, 2, 20, -1, 1);
      p1_refCorrelations[i] = new TProfile((TString("refCorrelations_sub")+=i).Data(), "",
                                           8, 0, 8, "s");
      h1_refBinSumW2[i] = new TH1F((TString("refBinSumW2_sub")+=i).Data(), "",
                                   8, 0, 8);
      p3_C14[i] = new TProfile3D((TString("C14_sub")+=i).Data(), "",
                                 5, 0, 5, 40, 0, 2, 20, -1, 1, "s");
      p3_C15[i] = new TProfile3D((TString("C15_sub")+=i).Data(), "",
                                 5, 0, 5, 40, 0, 2, 20, -1, 1, "s");
      p3_T28[i] = new TProfile3D((TString("T28_sub")+=i).Data(), "",
                                 5, 0, 5, 40, 0, 2, 20, -1, 1, "s");
      h3_DiffBinSumW12[i]= new TH3F((TString("diffBinSumW12_sub")+=i).Data(), "",
                                    5, 0, 5, 40, 0, 2, 20, -1, 1);
      h3_DiffBinSumW22[i]= new TH3F((TString("diffBinSumW22_sub")+=i).Data(), "",
                                    5, 0, 5, 40, 0, 2, 20, -1, 1);
      p3_T16abT28[i]= new TProfile3D((TString("T16abT28_sub")+=i).Data(), "",
                                     5, 0, 5, 40, 0, 2, 20, -1, 1);
    }
  ...
  for (int iEvent = 0; iEvent < nEvents; ++iEvent)
    {
      ...
      if (!m1 || !m2) continue;
      /// 五个bin里填写的内容
      p1_subEvent_refCorr->Fill((double)0,
                                (q1*TComplex::Conjugate(q2)).Re()/(m1*m2), m1*m2);
                  ...
                  for (Int_t iEta = etaBin[iLoop]; iEta <= etaBin[iLoop+2]; ++iEta)
                    {
                      ...
                      /// 给出POI粒子的单个事件总的cos_nPhi, sin_nPhi和粒子数
                      ...
                      p3_T16abT28[iLoop]->Fill(binPOI, binPt, binEta,
                                               ((q1*TComplex::Conjugate(q2)).Re()
                                                /(m1*m2))*(diffCoor22/diffNComb2),
                                               (m1*m2)*diffWeight2);
                    }//for
                }//for
            }//for
...
bool IsRFP(float eta, float pt, int loop)
{
  ...
  /// (-infinity, -1.3) || (0.3, +infinity)
  if (loop==0 && (eta>0.3 || eta<-1.3)) return true;
  /// (-infinity, -0.3) || (1,3, +infinity)
...
bool IsPOI(float eta, float pt, int id, int loop)
{
  ...
  /// (0, 1)
  else if (loop==1 && eta>0 && eta<1 && id==2212) return true;
