              ...
              for (unsigned w = t+1; w != tracks; ++w)
                {
                  if((ampt->ID[w] == 2 && ampt->ID[t] == -1)
                     || (ampt->ID[w] == -1 && ampt->ID[t] == 2))
                    {
                      ...
                      double omega = (3./4.)* (tmass*tmass + wmass*wmass) / (tmass*wmass*(tmass+wmass)*RRMS*RRMS);
                      ...
                      double Fb = G * 8 * exp(-rhoSquare/sigmarhoSquare - krhoSquare*sigmarhoSquare);
                      // cout << Fb << endl;
