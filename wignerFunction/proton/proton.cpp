  ...
  for (unsigned l = 0; l != entries; ++l)
    {
      ...
      for (unsigned t = 0; t != tracks-2; ++t)
        {
          if (ampt->ID[t] == 2 || ampt->ID[t] == 1)
            {
              // cout << l << "  ======>  " << ampt->ID[t] << endl;
              for (unsigned w = t+1; w != tracks-1; ++w)
                {
                  if((ampt->ID[w] == 2 && ampt->ID[t] == 2)
                     || (ampt->ID[w] == 2 && ampt->ID[t] == 1)
                     || (ampt->ID[w] == 1 && ampt->ID[t] == 2))
                    {
                      for (unsigned int h = w+1; h != tracks; ++h)
                        {
                          if ((ampt->ID[t] == 2 && ampt->ID[w] == 2 && ampt->ID[h] == 1)
                              || (ampt->ID[t] == 2 && ampt->ID[w] == 1 && ampt->ID[h] == 2)
                              || (ampt->ID[t] == 1 && ampt->ID[w] == 2 && ampt->ID[h] == 2))
                            {
                              ...
                              TVector3 rho = 1./sqrt(2.) * (tcencoordinate3D - wcencoordinate3D);
                              TVector3 krho = sqrt(2.) * (wmass*tcenmomentum3D - tmass*wcenmomentum3D) * (1. / (tmass+wmass));
                              TVector3 lambda = sqrt(2./3.) * ((tmass*tcencoordinate3D+wmass*wcencoordinate3D)
                                                               * (1./(tmass+wmass)) - hcencoordinate3D);
                              TVector3 klambda = sqrt(3./2) * ((hmass*(tcenmomentum3D+wcenmomentum3D)
                                                              -(tmass+wmass)*hcenmomentum3D)
                                                               *(1./(tmass+wmass+hmass)));
                             double omega = 0.5 * (wmass*hmass*(wmass+hmass) +tmass*hmass*(tmass+hmass)
                                             + tmass*wmass*(tmass+wmass))
                               / ((tmass+wmass+hmass)*tmass*wmass*hmass*RRMS*RRMS);
                             double mu_1 = 2.*pow(1./tmass + 1./wmass, -1.);
                             double mu_2 = (3./2) * pow(1./(tmass+wmass) + 1./hmass, -1.);
                             double sigmarho = 1. / sqrt(mu_1*omega);
                             double sigmarlambda = 1. / sqrt(mu_2*omega);
                             double rhoSquare = rho.Mag2();
                             rhoSquare = 25.6889 * rhoSquare;   //GeV
                             double lambdaSquare = lambda.Mag2();
                             lambdaSquare = 25.6889 * lambdaSquare; //GeV
                             double krhoSquare = krho.Mag2();
                             double klambdaSquare = klambda.Mag2();
                             double sigmarhoSquare = sigmarho * sigmarho;
                             double sigmalambdaSquare = sigmarlambda * sigmarlambda;
                             ...
                             // cout << rhoSquare/sigmarhoSquare << endl;
                             // cout << lambdaSquare/sigmalambdaSquare << endl;
                             // cout << krhoSquare*sigmarhoSquare << endl;
                             // cout << klambdaSquare*sigmalambdaSquare << endl;

                              double Fb = G * 8 * 8 * exp(-rhoSquare/sigmarhoSquare - lambdaSquare/sigmalambdaSquare
                                                          - krhoSquare*sigmarhoSquare - klambdaSquare*sigmalambdaSquare);
                             // if (Fb > 1e-10)
                             //   {
                             //     cout << Fb << endl;
                             //   } 
